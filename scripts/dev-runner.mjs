...
function startProcess(index) {
  ...
  const child = spawn(command, args, { stdio: ['inherit', 'pipe', 'pipe'], env: process.env, shell: true });
  children.push(child);
...
function buildOnce() {
  const result = spawnSync('npm', ['run', 'build:vault'], { stdio: 'inherit', env: process.env, shell: true });
  if (result.status !== 0) {
