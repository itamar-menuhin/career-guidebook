        ...
        <div className="container max-w-5xl space-y-4">
          ...
          <div className="relative">
            ...
            <Input
              ...
              className="pl-12 h-14 text-lg bg-background/80 border-border/50 focus-visible:border-primary/50"
            />
          </div>
          ...
          <div className="flex flex-wrap gap-x-8 gap-y-6">
            <div className="space-y-2.5">
              <p className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Focus Area</p>
              <div className="flex flex-wrap gap-2">
                {[
                  { id: 'ai-safety', label: 'AI Safety' },
                  { id: 'biosecurity', label: 'Biosecurity' },
                  { id: 'climate', label: 'Climate' },
                  { id: 'global-health-development', label: 'Global Health' },
                  { id: 'animal-welfare', label: 'Animal Welfare' },
                  { id: 'ai-x-animals', label: 'AI × Animals' },
                ].map(area => (
                  <Badge
                    key={area.id}
                    variant={filters.focusArea === area.id ? 'default' : 'outline'}
                    className={cn(
                      'cursor-pointer text-base font-normal px-3 py-1.5 transition-all duration-200',
                      filters.focusArea === area.id ? 'shadow-sm' : 'hover:bg-muted/60 border-border/50'
                    )}
                    onClick={() => setFilter('focusArea', filters.focusArea === area.id ? null : area.id)}
                  >
                    {area.label}
                  </Badge>
                ))}
            ...
            <div className="space-y-2.5">
              <p className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Topic</p>
              <div className="flex flex-wrap gap-2">
                {topicOptions.map(t => (
                  <Badge
                    key={t}
                    ...
                    className={cn(
                      'cursor-pointer capitalize text-base font-normal px-3 py-1.5 transition-all duration-200',
            ...
            <div className="space-y-2.5">
              <p className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Commitment</p>
              <div className="flex flex-wrap gap-2">
